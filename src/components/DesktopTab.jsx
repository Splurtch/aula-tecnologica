...
import {
  ...
  Terminal, Globe, Wifi, Download, Settings, Search, ChevronRight, HardDrive
} from 'lucide-react';
...
export default function DesktopTab() {
  ...
  const renderPanelExplicativo = () => {
    if (!selectedItem) {
      return (
        <div className="p-5 bg-gradient-to-br from-white via-slate-50 to-slate-100 border-t border-slate-200">
    ...
    return (
      <div className="border-t border-slate-200 bg-white">
        <div className={`px-6 py-4 bg-gradient-to-r from-slate-800 via-slate-800 to-slate-700 text-white`}>
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-lg ${colors.bg} bg-opacity-20 backdrop-blur-sm border border-white/10`}>
              {selectedItem.icon && <selectedItem.icon size={24} />}
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-bold text-white">{selectedItem.name}</h3>
              <span className="text-xs text-slate-300 uppercase tracking-wider">{selectedItem.category}</span>
            </div>
            <div className="flex gap-1.5">
              <span className="px-2.5 py-1 bg-white/10 rounded text-[10px] border border-white/20">Simple click</span>
              <span className="px-2.5 py-1 bg-white/10 rounded text-[10px] border border-white/20">Doble click</span>
              <span className="px-2.5 py-1 bg-white/10 rounded text-[10px] border border-white/20">Click derecho</span>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className={`rounded-lg border ${colors.border} ${colors.light} p-4`}>
                <h4 className={`text-xs font-bold uppercase tracking-widest ${colors.text} mb-3 flex items-center gap-2`}>
                  <FileText size={14} />
                  Descripción
                </h4>
            ...
            <div className="space-y-4">
                ...
                <ul className="space-y-2.5">
                  {selectedItem.details?.split('\n').map((line, i) => (
                    <li key={i} className="text-sm text-slate-600 flex items-start gap-2">
                      <span className={`mt-0.5 ${colors.text} font-bold`}>→</span>
                      {line.replace(/^[•\-]\s*/, '')}
                    </li>
                  ))}
                </ul>
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-4">
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg ${colors.light} border ${colors.border} flex items-center justify-center`}>
                    {selectedItem.icon && <selectedItem.icon size={20} className={colors.text} />}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-slate-700">{selectedItem.name}</p>
                    <p className="text-xs text-slate-400">
                      {selectedItem.id === 'this_pc' || selectedItem.id === 'files' || selectedItem.id === 'trash' ? 'Parte izquierda del escritorio' :
                       selectedItem.id === 'informe_txt' || selectedItem.id === 'foto_png' ? 'Parte derecha del escritorio' :
                       selectedItem.id === 'system_tray' || selectedItem.id === 'taskbar' ? 'Barra inferior del escritorio' : 'Elemento del sistema'}
                    </p>
                  </div>
                </div>
              </div>
  ...
  return (
    <div className="rounded-sm border bg-slate-100 border-slate-200 shadow-[0_22px_60px_rgba(15,23,42,0.12)] overflow-hidden">
      ...
      <div className="relative" style={{ height: '420px', background: 'linear-gradient(180deg, #1e3a5f 0%, #2d5a87 40%, #4a90a4 70%, #7ab8c9 100%)' }}>
        ...
        <AnimatePresence>
          {showInformeWindow && (
            <VentanaDocumento onClose={() => setShowInformeWindow(false)} />
          )}
        </AnimatePresence>

        <AnimatePresence>
          {showFotoWindow && (
            <VentanaImagen onClose={() => setShowFotoWindow(false)} />
          )}
        </AnimatePresence>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-14 bg-white/95 backdrop-blur border-t border-slate-300 flex items-center px-3">
          <button onClick={() => setShowStartMenu(!showStartMenu)} className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-slate-200 transition-colors">
            <Layers size={20} className="text-violet-600" />
            <span className="font-semibold text-slate-700">Inicio</span>
          </button>
          <div className="w-px h-8 bg-slate-300 mx-2"></div>
          <button onClick={() => setShowExplorer(true)} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-100 text-slate-700">
      ...
      <div className="mx-4 mb-4 rounded-sm border border-amber-500/30 bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-amber-500/10 p-4">
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <a href="https://buymeacoffee.com/digitalsynapse" target="_blank" rel="noopener noreferrer" className="shrink-0 px-4 py-2 rounded-sm bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white font-bold text-xs uppercase tracking-wider transition-all hover:-translate-y-0.5 shadow-lg shadow-amber-500/20">
            Invítame a un café
          </a>
        </div>
      </div>
...
function DesktopIcon({ item, isSelected, onClick, onDoubleClick, onContextMenu, draggable, onDrop, onDragData }) {
  ...
  return (
    <motion.button
      ...
      draggable={draggable}
      onDragStart={(e) => {
        if (draggable) e.dataTransfer.setData('text/plain', onDragData || item.id);
      }}
      onDragOver={(e) => { if (onDrop) { e.preventDefault(); setIsDragOver(true); } }}
      onDragLeave={() => setIsDragOver(false)}
      onDrop={(e) => {
        if (onDrop) { e.preventDefault(); const id = e.dataTransfer.getData('text/plain'); onDrop(id); setIsDragOver(false); }
      }}
      onClick={onClick}
      onDoubleClick={onDoubleClick}
      onContextMenu={onContextMenu}
      className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-all cursor-pointer group ${isSelected ? 'bg-white/20 ring-2 ring-white/50' : 'hover:bg-white/10'} ${isDragOver ? 'bg-rose-400/50 scale-110' : ''}`}
    >
      <div className={`w-14 h-14 rounded-lg flex items-center justify-center shadow-lg transition-all ${isSelected ? 'bg-white/30 ring-2 ring-white' : 'bg-white/20 group-hover:bg-white/25'}`}>
        <item.icon size={32} className="text-white drop-shadow-lg" />
      </div>
      <span className="text-[11px] font-medium text-white text-center drop-shadow-md max-w-[60px] truncate">{item.name}</span>
    </motion.button>
  );
}
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-r from-violet-600 to-purple-600 px-4 py-6 text-white">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-white/20 flex items-center justify-center"><Layers size={24} /></div>
          <div>
      ...
      <div className="p-2">
        <div className="relative mb-3">
          ...
          <input type="text" placeholder="Buscar..." className="w-full pl-10 pr-4 py-2 bg-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
        </div>
        <div className="grid grid-cols-3 gap-1">
          {[
            ...
            <button key={i} onClick={app.action} className="flex flex-col items-center gap-1 p-3 rounded-lg hover:bg-slate-100 transition-colors">
    ...
    >
      ...
      <div className="p-4 grid grid-cols-2 gap-3">
        {carpetas.map((carpeta) => (
          <button key={carpeta.id} onClick={() => onSelect({ ...desktopData.files, name: carpeta.name })}
...
function VentanaDocumento({ onClose }) {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.8, rotate: -5 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} exit={{ opacity: 0, scale: 0.8, rotate: 5 }}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-white">Este PC - Discos y Dispositivos</span>
      </div>
      <div className="grid grid-cols-3 gap-3">
        {[
          { letter: 'C:', name: 'Disco local', size: '256 GB', color: 'blue', used: 180 },
          { letter: 'D:', name: 'Datos', size: '512 GB', color: 'emerald', used: 320 },
          { letter: 'E:', name: 'USB', size: '32 GB', color: 'amber', used: 8 },
        ].map((drive) => (
          <div key={drive.letter} className="bg-slate-800 rounded-lg p-3 border border-slate-700">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-white font-medium text-sm">{drive.letter}</span>
            </div>
            <p className="text-slate-400 text-xs mb-2">{drive.name}</p>
            <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
              <div className={`h-full bg-${drive.color}-500 rounded-full`} style={{ width: `${(drive.used / parseInt(drive.size)) * 100}%` }}></div>
            </div>
            <p className="text-slate-500 text-[10px] mt-1">{drive.used} GB / {drive.size}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-white">Carpeta de Archivos Personales</span>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <button key={folder.name} onClick={onOpenExplorer}
            className="flex items-center gap-2 bg-slate-800 rounded-lg p-2 border border-slate-700 hover:border-slate-600 transition-colors text-left">
            <folder.icon size={16} className={`text-${folder.color}-400`} />
            <div>
    ...
    <div className="space-y-4">
      ...
      {items.length === 0 ? (
        <div className="text-center py-6 text-slate-500">
          ...
          <p className="text-xs text-slate-600 mt-1">Arrastra archivos aquí para eliminarlos</p>
        </div>
      ) : (
        <div className="space-y-2">
          {items.map((item, i) => (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-white">Bloc de Notas - Documento de Texto</span>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-white">Visor de Fotografías</span>
      </div>
      <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
        ...
        <div className="mt-3 flex gap-2 justify-center">
          ...
          <span className="px-2 py-1 bg-slate-700 rounded text-xs text-slate-300">2.4 MB</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-white">Menú Inicio de Windows</span>
      </div>
      <div className="bg-slate-800 rounded-lg p-3 border border-slate-700">
        <div className="bg-gradient-to-r from-violet-600 to-purple-600 rounded-lg px-4 py-3 mb-3">
          <div className="flex items-center gap-2 text-white">
            <Layers size={20} />
            <span className="font-bold">Windows 12</span>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <button key={i} className="flex flex-col items-center gap-1 p-2 bg-slate-700 rounded-lg hover:bg-slate-600">
              <app.icon size={20} className={app.color} />
              <span className="text-[9px] text-slate-300">{app.name}</span>
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-white">Barra de Tareas de Windows</span>
      </div>
      <div className="bg-slate-800 rounded-lg p-2 flex items-center gap-2 border border-slate-700">
        <div className="w-9 h-9 rounded bg-violet-600 flex items-center justify-center"><Layers size={18} className="text-white" /></div>
        <div className="w-9 h-9 rounded bg-amber-500 flex items-center justify-center"><FolderOpen size={18} className="text-white" /></div>
        <div className="w-9 h-9 rounded bg-blue-500 flex items-center justify-center"><Globe size={18} className="text-white" /></div>
        <div className="flex-1"></div>
        <div className="flex items-center gap-2">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-white">Zona de Notificaciones y Sistema</span>
      </div>
      <div className="bg-slate-800 rounded-lg p-3 border border-slate-700 space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Wifi size={14} className="text-emerald-400" />
            <span className="text-xs text-slate-300">WiFi - Red domestica</span>
          </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-700">
          ...
          <span className="text-xs text-slate-300">{new Date().toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
      </div>
...
function SimulatorIconos() {
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Monitor size={20} className="text-blue-400" />
        <span className="font-semibold text-white">Accesos Directos del Escritorio</span>
      </div>
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={item.name} className="bg-slate-800 rounded-lg p-3 border border-slate-700 text-center">
            <item.icon size={28} className={`mx-auto text-${item.color}-400 mb-1`} />
...
function SimulatorBotonesVentana() {
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Monitor size={20} className="text-slate-400" />
        <span className="font-semibold text-white">Botones de Control de Ventana</span>
      </div>
      <div className="flex justify-center gap-3">
        ...
        <div className="w-12 h-12 rounded-lg bg-red-500 flex items-center justify-center border border-red-600">
          <span className="text-white text-xl">✕</span>
        </div>
      </div>
      <div className="space-y-2 text-xs text-slate-400">
        <div className="flex items-center gap-3 justify-center">
          <span className="w-8 h-8 rounded bg-slate-700 flex items-center justify-center">─</span>
        ...
        <div className="flex items-center gap-3 justify-center">
          <span className="w-8 h-8 rounded bg-red-500 flex items-center justify-center text-white">✕</span>
